"""Thin JSON bridge for the TypeScript bindings."""
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        response = dispatch[request.operation](request) if request.operation != "discover_models" else kernel.discover_models()
    except Exception as exc:  # pragma: no cover - defensive bridge fallback
        response = kernel.KernelResponse(
            operation=str(request_data.get("operation", "discover_models")) if "request_data" in locals() else "discover_models",

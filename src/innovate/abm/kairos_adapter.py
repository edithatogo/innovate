...
class KairosSimulationAdapter:
    ...
    def _run_reference(self, request: KairosSimulationRequest) -> KairosSimulationResult:
        ...
        for stream in request.random_streams:
            # Mix configured stream identifiers with a stable (non-hash()) seed.
            mixed = (
                request.seed.primary
                ^ _stable_seed_token(stream.name)
                ^ _stable_seed_token(stream.seed.stream_id)
            ) & 0xFFFFFFFF
        ...
        # Map agents onto topology nodes round-robin when counts differ.
        node_ids = list(request.topology.node_ids)
        agent_nodes = {
            agent_id: node_ids[index % len(node_ids)]
            for index, agent_id in enumerate(agent_states)
        }
        ...
        for step in range(1, request.steps + 1):
            ...
            # DES resource queue: arrivals then optional service completion.
            ...
            # ABM behavior: susceptible agents adopt based on neighbor pressure.
            for agent_id, agent in list(agent_states.items()):
                ...
                for neighbor, weight in neighbors:
                    neighbor_agents = [
                        ...
                        if other_node == neighbor and agent_states[other_id].state == "adopted"
                    ]
                    if neighbor_agents:
            ...
            for node in node_ids:
                node_agents = [
                    agent_id for agent_id, other_node in agent_nodes.items() if other_node == node
                ]
                if not node_agents:
        ...
        diagnostics = {
            ...
            "bridge_promoted": self.evidence.claims_promoted_bridge(),
            "fallback_reason": (
                "bridge crates gated; using deterministic contract reference engine"
            ),

...
def validate_float(value: Union[float, int], param_name: str, min_val: Optional[float] = None, max_val: Optional[float] = None) -> float:
...
def validate_probability(value: Union[float, int], param_name: str) -> float:
    """Validate that a value is a probability (between 0 and 1)."""
    return validate_float(value, param_name, min_val=0.0, max_val=1.0)


def validate_covariates(covariates: Optional[Sequence[str]], param_name: str = "covariates") -> Sequence[str]:
...
def validate_covariates_dict(covariates_dict: Optional[Dict[str, Sequence]], expected_covariates: Sequence[str], t_length: int) -> Optional[Dict[str, np.ndarray]]:

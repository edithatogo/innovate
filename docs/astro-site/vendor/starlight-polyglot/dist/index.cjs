...
__export(index_exports, {
  createPolyglotPlugin: () => createPolyglotPlugin,
  default: () => polyglot,
...
var import_github_slugger = require("github-slugger");
function transformToMDX(modules, options) {
  const pages = [];
  const sidebarItems = [];
  const { outputDir, language } = options;
  for (const mod of modules) {
    const modSlug = (0, import_github_slugger.slug)(mod.name);
    ...
    for (const cls of mod.classes ?? []) {
      const clsSlug = `${modSlug}.${(0, import_github_slugger.slug)(cls.name)}`;
      pages.push({
    ...
    for (const fn of mod.functions ?? []) {
      const fnSlug = `${modSlug}.${(0, import_github_slugger.slug)(fn.name)}`;
      pages.push({
...
function generateModuleBody(mod) {
  ...
  if (mod.classes && mod.classes.length > 0) {
    parts.push("## Classes", "");
    for (const cls of mod.classes) {
      parts.push(`- [${cls.name}](${(0, import_github_slugger.slug)(mod.name)}.${(0, import_github_slugger.slug)(cls.name)}/) ${cls.docstring?.split("\n")[0] ?? ""}`);
    }
    parts.push("");
  }
  if (mod.functions && mod.functions.length > 0) {
    ...
    for (const fn of mod.functions) {
      parts.push(`- [${fn.name}](${(0, import_github_slugger.slug)(mod.name)}.${(0, import_github_slugger.slug)(fn.name)}/)
  ${fn.docstring?.split("\n")[0] ?? ""}`);
    }
    parts.push("");
  }
...
var cppHandler = {
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process.execSync)("doxygen --version", { encoding: "utf-8", stdio: "pipe" });
...
function extractWithDoxygen(projectPath, doxyfilePath, inputDirs) {
  const resolvedPath = import_node_path2.default.resolve(projectPath);
  const outputDir = import_node_path2.default.resolve(resolvedPath, "doxygen_xml");
  const doxyfile = doxyfilePath ? import_node_path2.default.resolve(doxyfilePath) : import_node_path2.default.resolve(resolvedPath, "Doxyfile");
  if (!(0, import_node_fs.existsSync)(doxyfile)) {
    generateMinimalDoxyfile(resolvedPath, outputDir, inputDirs);
  }
  const doxygenCmd = `doxygen "${doxyfile}" 2>&1`;
  (0, import_node_child_process.execSync)(doxygenCmd, {
    encoding: "utf-8",
    cwd: resolvedPath,
    stdio: "pipe",
    timeout: 18e4
...
function generateMinimalDoxyfile(projectPath, xmlOutputDir, inputDirs) {
  const input = inputDirs && inputDirs.length > 0 ? inputDirs.map((d) => import_node_path2.default.resolve(projectPath, d)).join(" ") : projectPath;
  ...
  (0, import_node_fs.writeFileSync)(import_node_path2.default.join(projectPath, "Doxyfile"), doxyfileContent, "utf-8");
}
function parseDoxygenXml(xmlDir) {
  const modules = [];
  const files = (0, import_node_fs.readdirSync)(xmlDir).filter(
    (f) => f.endsWith(".xml") && f !== "index.xml" && f !== "DoxygenLayout.xml"
  );
  for (const file of files) {
    const filePath = import_node_path2.default.join(xmlDir, file);
...
var csharpHandler = {
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process2.execSync)("dotnet --version", { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
...
function extractWithDotNet(projectPath, configuration = "Release", explicitDocPath) {
  ...
  (0, import_node_child_process2.execSync)(buildCmd, {
    encoding: "utf-8",
    stdio: "pipe",
    timeout: 18e4
  });
  ...
  if (!xmlDocPath) {
    const projectName = import_node_path3.default.basename(resolvedPath, import_node_path3.default.extname(resolvedPath));
    const possiblePaths = [
      import_node_path3.default.resolve(resolvedPath, "bin", configuration, "net9.0", `${projectName}.xml`),
      import_node_path3.default.resolve(resolvedPath, "bin", configuration, "net8.0", `${projectName}.xml`),
      import_node_path3.default.resolve(resolvedPath, "bin", configuration, "net7.0", `${projectName}.xml`),
      import_node_path3.default.resolve(resolvedPath, "bin", configuration, "net6.0", `${projectName}.xml`),
      import_node_path3.default.resolve(resolvedPath, "bin", configuration, "netstandard2.0", `${projectName}.xml`)
    ];
    const binDir = import_node_path3.default.resolve(resolvedPath, "bin", configuration);
    if ((0, import_node_fs2.existsSync)(binDir)) {
      try {
        const frameworks = (0, import_node_fs2.readdirSync)(binDir, { withFileTypes: true });
        for (const fw of frameworks) {
          if (fw.isDirectory()) {
            possiblePaths.push(import_node_path3.default.resolve(binDir, fw.name, `${projectName}.xml`));
          }
        }
      } catch {
      }
  ...
  if (!xmlDocPath || !(0, import_node_fs2.existsSync)(xmlDocPath)) {
    throw new Error(
...
function parseXmlDocFile(xmlPath) {
  const xmlContent = (0, import_node_fs2.readFileSync)(xmlPath, "utf-8");
  const modules = [];
...
var dartHandler = {
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process3.execSync)("dart --version", { encoding: "utf-8", stdio: "pipe" });
      (0, import_node_child_process3.execSync)("dart doc --help", { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
      return {
        valid: false,
...
function extractWithDartdoc(entryPoints) {
  ...
  for (const entry of entryPoints) {
    const resolvedEntry = import_node_path4.default.resolve(entry);
    if (!(0, import_node_fs3.existsSync)(resolvedEntry)) {
      continue;
    }
    ...
    (0, import_node_child_process3.execSync)(cmd, {
      encoding: "utf-8",
      cwd: (0, import_node_fs3.existsSync)(import_node_path4.default.join(resolvedEntry, "pubspec.yaml")) ? resolvedEntry : import_node_path4.default.dirname(resolvedEntry),
      maxBuffer: 10 * 1024 * 1024,
      timeout: 18e4,
      stdio: "pipe"
    });
    if ((0, import_node_fs3.existsSync)(jsonOutputPath)) {
      const raw = (0, import_node_fs3.readFileSync)(jsonOutputPath, "utf-8");
      try {
...
var elixirHandler = {
  ...
  async generate(options) {
    ...
    if (!(0, import_node_fs4.existsSync)(projectPath)) {
      throw new Error(`Project path does not exist: ${projectPath}`);
    }
    const mixExsPath = import_node_path5.default.join(projectPath, "mix.exs");
    if (!(0, import_node_fs4.existsSync)(mixExsPath)) {
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process4.execSync)("elixir --version", { encoding: "utf-8", stdio: "pipe" });
      (0, import_node_child_process4.execSync)("mix --version", { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
      return {
        valid: false,
...
function extractWithExDoc(projectPath) {
  ...
  (0, import_node_child_process4.execSync)("mix deps.get", {
    encoding: "utf-8",
    cwd: resolvedPath,
    stdio: "pipe",
    timeout: 12e4
  ...
  (0, import_node_child_process4.execSync)(cmd, {
    encoding: "utf-8",
    cwd: resolvedPath,
    maxBuffer: 10 * 1024 * 1024,
    timeout: 18e4,
  ...
  const docDir = import_node_path5.default.join(resolvedPath, "doc");
  const jsonPaths = [
    import_node_path5.default.join(docDir, "ex_doc.json"),
    import_node_path5.default.join(docDir, "docs.json"),
    ...(0, import_node_fs4.existsSync)(docDir) ? (0, import_node_fs4.readdirSync)(docDir).filter((f) => f.endsWith(".json")).map((f) => import_node_path5.default.join(docDir, f)) : []
  ];
  for (const jsonPath of jsonPaths) {
    if ((0, import_node_fs4.existsSync)(jsonPath)) {
      const raw = (0, import_node_fs4.readFileSync)(jsonPath, "utf-8");
      try {
...
var goHandler = {
  ...
  async generate(options) {
    ...
    if (!(0, import_node_fs5.existsSync)(modulePath)) {
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process5.execSync)("gomarkdoc --version", { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
...
function extractWithGoMarkdoc(modulePath) {
  const resolvedPath = import_node_path6.default.resolve(modulePath);
...
var javaHandler = {
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process6.execSync)("javadoc --version", { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
      return {
        valid: false,
...
function extractWithJavadoc(entryPoints, classpath) {
  const tmpDir = (0, import_node_fs6.mkdtempSync)(import_node_path7.default.join((0, import_node_os.tmpdir)(), "javadoc-json-"));
  try {
    ...
    for (const entry of entryPoints) {
      const resolved = import_node_path7.default.resolve(entry);
    ...
    (0, import_node_child_process6.execSync)(cmd, {
      encoding: "utf-8",
      stdio: "pipe",
      maxBuffer: 10 * 1024 * 1024,
      timeout: 12e4
    });
    const files = (0, import_node_fs6.readdirSync)(tmpDir);
    ...
    for (const jsonFile of jsonFiles) {
      const jsonPath = import_node_path7.default.join(tmpDir, jsonFile);
      const raw = (0, import_node_fs6.readFileSync)(jsonPath, "utf-8");
    ...
    try {
      (0, import_node_child_process6.execSync)(`rm -rf "${tmpDir}"`, { stdio: "pipe" });
    } catch {
    }
  }
}
...
var juliaHandler = {
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process7.execSync)("julia --version", { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
...
function extractWithJulia(entryPoints) {
  const scriptPath = import_node_path8.default.resolve(__dirname, "..", "scripts", "julia_extract.jl");
  ...
  const result = (0, import_node_child_process7.execSync)(args.join(" "), {
    encoding: "utf-8",
    maxBuffer: 10 * 1024 * 1024,
    // 10MB
    timeout: 12e4
...
var kotlinHandler = {
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process8.execSync)("dokka --version", { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
      try {
        (0, import_node_child_process8.execSync)("gradle --version", { encoding: "utf-8", stdio: "pipe" });
        return { valid: true, errors: [] };
      } catch {
        return {
          valid: false,
...
function extractWithDokka(projectPath, _outputFormat) {
  const resolvedPath = import_node_path9.default.resolve(projectPath);
  const outputDir = import_node_path9.default.resolve(resolvedPath, "build", "dokka", "json");
  const gradleBuildFiles = ["build.gradle.kts", "build.gradle", "pom.xml"];
  const hasGradle = gradleBuildFiles.some(
    (f) => (0, import_node_fs8.existsSync)(import_node_path9.default.resolve(resolvedPath, f))
  );
  if (hasGradle) {
...
function runGradleDokka(projectPath) {
  const gradleCmd = (0, import_node_fs8.existsSync)(import_node_path9.default.join(projectPath, "gradlew")) ? "./gradlew" : "gradle";
  const cmd = `${gradleCmd} dokkaJson --no-daemon 2>&1`;
  (0, import_node_child_process8.execSync)(cmd, {
    encoding: "utf-8",
    cwd: projectPath,
    stdio: "pipe",
    timeout: 3e5
...
function findKotlinSourceDirs(projectPath) {
  ...
  for (const candidate of candidates) {
    const fullPath = import_node_path9.default.resolve(projectPath, candidate);
...
function parseDokkaOutput(outputDir) {
  const modules = [];
  const files = (0, import_node_fs8.readdirSync)(outputDir).filter((f) => f.endsWith(".json"));
  for (const file of files) {
    const filePath = import_node_path9.default.join(outputDir, file);
    const raw = (0, import_node_fs8.readFileSync)(filePath, "utf-8");
    const data = JSON.parse(raw);
    const moduleName = data.module ?? import_node_path9.default.basename(file, ".json");
    const mod = {
      name: moduleName,
      docstring: void 0,
      classes: [],
...
var phpHandler = {
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process9.execSync)("phpdoc --version", { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
      return {
        valid: false,
...
function extractWithPhpDoc(entryPoints) {
  ...
  const entries = entryPoints.map((e) => `"${import_node_path10.default.resolve(e)}"`).join(" ");
  const cmd = `phpdoc -t "${tmpOutput}" --template="xml" -f ${entries} -d "" 2>&1`;
  (0, import_node_child_process9.execSync)(cmd, {
    encoding: "utf-8",
    maxBuffer: 10 * 1024 * 1024,
    timeout: 18e4,
    stdio: "pipe"
  });
  if ((0, import_node_fs9.existsSync)(tmpOutput)) {
    const xmlFiles = (0, import_node_fs9.readdirSync)(tmpOutput).filter((f) => f.endsWith(".xml")).map((f) => import_node_path10.default.join(tmpOutput, f));
    for (const xmlFile of xmlFiles) {
      const raw = (0, import_node_fs9.readFileSync)(xmlFile, "utf-8");
...
var pythonHandler = {
  ...
  async validate(sourcePath) {
    try {
      const result = (0, import_node_child_process10.execSync)('python3 -c "import griffe"', { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
...
function extractWithGriffe(entryPoints, pythonExecutable = process.env.STARLIGHT_POLYGLOT_PYTHON ?? "python3") {
  const scriptPath = import_node_path11.default.resolve(__dirname, "..", "scripts", "python_extract.py");
  ...
  const result = (0, import_node_child_process10.execSync)(args.join(" "), {
    encoding: "utf-8",
    maxBuffer: 10 * 1024 * 1024,
    // 10MB
    timeout: 6e4
...
var rHandler = {
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process11.execSync)("Rscript --version", { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
...
function extractWithRScript(entryPoints) {
  const scriptPath = import_node_path12.default.resolve(__dirname, "..", "scripts", "r_extract.R");
  ...
  const result = (0, import_node_child_process11.execSync)(args.join(" "), {
    encoding: "utf-8",
    maxBuffer: 10 * 1024 * 1024,
    // 10MB
    timeout: 12e4
...
var rubyHandler = {
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process12.execSync)("yard --version", { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
      return {
        valid: false,
...
function extractWithYard(entryPoints) {
  const modules = [];
  for (const entry of entryPoints) {
    const resolvedEntry = import_node_path13.default.resolve(entry);
    if (!(0, import_node_fs12.existsSync)(resolvedEntry)) {
      continue;
    }
    const tmpOutput = "/tmp/starlight-polyglot-yard";
    const cmd = `yard doc --output-format json --output "${tmpOutput}" "${resolvedEntry}" 2>&1`;
    (0, import_node_child_process12.execSync)(cmd, {
      encoding: "utf-8",
      cwd: (0, import_node_fs12.existsSync)(resolvedEntry) && resolvedEntry.endsWith(".rb") ? import_node_path13.default.dirname(resolvedEntry) : resolvedEntry,
      maxBuffer: 10 * 1024 * 1024,
      timeout: 18e4,
      stdio: "pipe"
    });
    const yardJsonPath = import_node_path13.default.join(tmpOutput, "yard.json");
    if ((0, import_node_fs12.existsSync)(yardJsonPath)) {
      const raw = (0, import_node_fs12.readFileSync)(yardJsonPath, "utf-8");
      try {
...
var rustHandler = {
  ...
  async generate(options) {
    ...
    if (!(0, import_node_fs13.existsSync)(cratePath)) {
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process13.execSync)("cargo --version", { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
...
function extractWithRustDoc(cratePath) {
  const resolvedPath = import_node_path14.default.resolve(cratePath);
  const cmd = `cargo +nightly rustdoc --output-format json --manifest-path "${resolvedPath}/Cargo.toml" 2>/dev/null`;
  (0, import_node_child_process13.execSync)(cmd, {
    encoding: "utf-8",
    stdio: "pipe",
    timeout: 12e4
  });
  const crateName = import_node_path14.default.basename(resolvedPath);
  const possiblePaths = [
    import_node_path14.default.resolve(resolvedPath, "target", "doc", `${crateName}.json`),
    import_node_path14.default.resolve(resolvedPath, "..", "target", "doc", `${crateName}.json`)
  ];
  let jsonPath = "";
  for (const p of possiblePaths) {
  ...
  const raw = (0, import_node_fs13.readFileSync)(jsonPath, "utf-8");
...
var sasHandler = {
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process14.execSync)("sas -version", { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
      try {
        (0, import_node_child_process14.execSync)("sas --version", { encoding: "utf-8", stdio: "pipe" });
...
function extractWithSas(entryPoints) {
  const modules = [];
  const scriptPath = import_node_path15.default.resolve(__dirname, "..", "scripts", "sas_extract.sas");
  if (!(0, import_node_fs14.existsSync)(scriptPath)) {
    throw new Error(`SAS extraction script not found at ${scriptPath}`);
  }
  for (const entry of entryPoints) {
    const resolvedEntry = import_node_path15.default.resolve(entry);
    if (!(0, import_node_fs14.existsSync)(resolvedEntry)) {
      continue;
    }
    const cmd = `sas -sysin "${scriptPath}" -set SRC_FILE "${resolvedEntry}" -log /tmp/sas_extract.log -print /tmp/sas_extract.lst 2>&1`;
    const result = (0, import_node_child_process14.execSync)(cmd, {
      encoding: "utf-8",
...
function parseSasOutput(output, entryPath) {
  const name = import_node_path15.default.basename(entryPath, import_node_path15.default.extname(entryPath));
...
var scalaHandler = {
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process15.execSync)("scaladoc --version", { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
      return {
        valid: false,
...
function extractWithScaladoc(entryPoints, classpath) {
  ...
  const entries = entryPoints.map((e) => `"${import_node_path16.default.resolve(e)}"`).join(" ");
  const cpFlag = classpath ? ` -classpath "${classpath}"` : "";
  const cmd = `scaladoc -d "${tmpDir}" -json${cpFlag} ${entries} 2>&1`;
  (0, import_node_child_process15.execSync)(cmd, {
    encoding: "utf-8",
    maxBuffer: 10 * 1024 * 1024,
    timeout: 18e4,
    stdio: "pipe"
  });
  if (!(0, import_node_fs15.existsSync)(tmpDir)) {
    throw new Error(
...
function parseScaladocDir(dir) {
  ...
  for (const file of files) {
    const filePath = import_node_path16.default.join(dir, file);
...
var stataHandler = {
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process16.execSync)("stata --version", { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
      try {
        (0, import_node_child_process16.execSync)("stata-se --version", { encoding: "utf-8", stdio: "pipe" });
        return { valid: true, errors: [] };
      } catch {
        return {
          valid: false,
...
function extractWithStata(entryPoints) {
  const modules = [];
  const scriptPath = import_node_path17.default.resolve(__dirname, "..", "scripts", "stata_extract.do");
  if (!(0, import_node_fs16.existsSync)(scriptPath)) {
    throw new Error(`Stata extraction script not found at ${scriptPath}`);
  }
  for (const entry of entryPoints) {
    const resolvedEntry = import_node_path17.default.resolve(entry);
    if (!(0, import_node_fs16.existsSync)(resolvedEntry)) {
      continue;
    }
    const cmd = `stata -b do "${scriptPath}" "${resolvedEntry}"`;
    const result = (0, import_node_child_process16.execSync)(cmd, {
      encoding: "utf-8",
...
function parseStataHelpOutput(output, entryPath) {
  const name = import_node_path17.default.basename(entryPath, import_node_path17.default.extname(entryPath));
...
var swiftHandler = {
  ...
  async generate(options) {
    ...
    if (!(0, import_node_fs17.existsSync)(modulePath)) {
      throw new Error(`Module path does not exist: ${modulePath}`);
    }
  ...
  async validate(_sourcePath) {
    try {
      (0, import_node_child_process17.execSync)("swift-doc --version", { encoding: "utf-8", stdio: "pipe" });
      return { valid: true, errors: [] };
    } catch {
      try {
        (0, import_node_child_process17.execSync)("swift --version", { encoding: "utf-8", stdio: "pipe" });
        return { valid: true, errors: [] };
      } catch {
        return {
          valid: false,
...
function extractWithSwiftDoc(modulePath, symbolGraphDir) {
  const resolvedPath = import_node_path18.default.resolve(modulePath);
  if (symbolGraphDir && (0, import_node_fs17.existsSync)(symbolGraphDir)) {
  ...
  if (symbolGraphCandidates.length > 0) {
    return parseSymbolGraphDir(import_node_path18.default.dirname(symbolGraphCandidates[0]));
  }
...
function findSymbolGraphFiles(modulePath) {
  ...
  const symbolGraphDir = import_node_path18.default.join(modulePath, ".build", "symbolgraph");
  if ((0, import_node_fs17.existsSync)(symbolGraphDir)) {
...
function runSwiftDoc(modulePath) {
  const tmpOutput = import_node_path18.default.resolve(modulePath, ".build", "swift-doc-output");
  const cmd = `swift-doc --output "${tmpOutput}" --format json "${modulePath}" 2>&1`;
  (0, import_node_child_process17.execSync)(cmd, {
    encoding: "utf-8",
    cwd: modulePath,
    stdio: "pipe",
    timeout: 18e4
  });
  if (!(0, import_node_fs17.existsSync)(tmpOutput)) {
    throw new Error(
...
function parseSymbolGraphDir(symbolGraphDir) {
  const modules = [];
  const files = (0, import_node_fs17.readdirSync)(symbolGraphDir).filter(
    (f) => f.endsWith(".json") || f.endsWith(".symbolgraph.json")
  );
  for (const file of files) {
    const filePath = import_node_path18.default.join(symbolGraphDir, file);
...
function createPolyglotPlugin() {
  const group = getSidebarGroupPlaceholder(Symbol((0, import_node_crypto.randomBytes)(24).toString("base64url")));

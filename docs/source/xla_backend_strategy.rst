...
* **Schema compatibility:** request, response, diagnostic, provenance, and
  error payloads remain compatible with the functional kernel contract.
...
* **No ABI leakage:** XLA lowering details, ``jaxlib`` internals, and exported
  JAX artifacts do not become the public compatibility contract.
...
* Probabilistic inference should prefer NumPyro or BlackJAX for eligible
  kernels before selecting a non-XLA probabilistic runtime.

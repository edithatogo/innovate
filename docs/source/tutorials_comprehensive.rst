Tutorials
=========

These are the canonical tutorials for the contract-first Innovate architecture.
They live under ``docs/source/tutorials/`` and are organized by use case rather
than by old notebook names.

Core Contract
-------------

.. toctree::
   :maxdepth: 1

   tutorials/functional_kernel_contract
   tutorials/diagnostics_contract
   tutorials/arrow_interchange

Modeling Workflows
------------------

.. toctree::
   :maxdepth: 1

   tutorials/norton_bass
   tutorials/advanced_diffusion_inference
   tutorials/bayesian_fitter_tutorial
   tutorials/multi_product
   tutorials/seasonal_data_tutorial
   tutorials/counterfactual_analysis

Integration and Bindings
------------------------

.. toctree::
   :maxdepth: 1

   tutorials/jax_backend
   tutorials/ndlib_integration

Further Reading
---------------

- :doc:`bindings`
- :doc:`innovate.models`
- :doc:`index`

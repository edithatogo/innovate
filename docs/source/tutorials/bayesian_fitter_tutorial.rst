...
For a deeper understanding, we can use libraries like ``ArviZ`` to plot the posterior distributions and diagnostic plots. The ``fitter.trace`` object is an ``arviz.InferenceData`` object that can be used directly with ``ArviZ``.

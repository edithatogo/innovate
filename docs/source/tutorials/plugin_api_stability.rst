...
``innovate`` treats API stability as an explicit contract instead of a social convention.
...
The capability registries expose a ``stability_tier`` property so callers can inspect the
normalized tier without relying on string comparisons.

Extension manifests
-------------------

Local extensions can be described with ``innovate.ExtensionManifest`` and registered with
``innovate.register_extension``. The current contract validates:
...
Release notes and compatibility reviews should name the tier of any new surface and explain
whether it is stable, provisional, or internal-only. That keeps plugin authors and downstream
bindings aligned with semver expectations before new extension points are promoted.
